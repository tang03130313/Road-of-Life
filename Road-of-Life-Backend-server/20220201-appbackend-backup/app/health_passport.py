...
def get_health_passport_ios(request):
    print(request.method)
    if(request.method == 'POST'):
        email = request.POST.get('email', '')
        password = request.POST.get('password', '')
...
def insert_health_passport_ios(request):
    print(request.method, " insert_health_passport_ios")
    if(request.method == 'POST'):
        print(" insert_health_passport_ios")
        email = request.POST.get('email', '')
        password = request.POST.get('password', '')
        user_id = request.POST.get('user_id', '') 
        passport_json =  request.POST.get('passport_json', '')
        ...
        print(passport_json)
        passport_json = passport_json.replace("\\r\\n","")
        passport_json = passport_json.replace("\\","")
        passport_json = json.loads(passport_json)
        print(passport_json)
        now_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
...
def get_key_ios(request):
    print(request.method)
    if(request.method == 'GET'):
        data = {}
        data['result'] = "b433ca88bc914bd6ae7ab8afb7fb47c3"
        print(data['result'])
        response = json.dumps(data, sort_keys=True, indent=4, ensure_ascii=False)
        return HttpResponse(response)
